...
EQUALITY
esha khator (kaq8eg)
collaboration with alexander davis (wfy8cn)
-/
...
at first appear.
-/
/- Since w and z are of the same type T, we can use the 
...
-/
/- 
Assume we are given any arbituary object and if the propisition 
works for that number, it will work for all objects of the same 
type
-/
...
"the introduction rule of And" to a proof of 
(P ∧ Q) and a proof of R.
What remains, then, is to obtain these proofs.
But this is easily done by the application of
"the elimination rule of And" to "p_qr". QED. 
-/
